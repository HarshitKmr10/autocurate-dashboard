...
export function KPICard({
  ...
  const { value, trend, trendPercentage } = useMemo(() => {
    // The API response has this structure: { results: { data: [...], columns: [...] } }
    const queryResult = (data as any)?.results as QueryResult | undefined;

    if (!queryResult || !queryResult.data || queryResult.data.length === 0) {
      return { value: 0, trend: null, trendPercentage: 0 };
    }

    // The SQL queries return results with 'AS value', so extract the value directly
    const resultRow = queryResult.data[0];
    let calculatedValue = 0;

    if (resultRow && "value" in resultRow) {
      // SQL query returned a 'value' field (which is what our queries do)
      calculatedValue = Number(resultRow.value) || 0;
    } else {
      ...
      if (column === "*") {
        // Count all records
        calculatedValue = queryResult.data.length;
      } else {
    ...
    const previousQueryResult = (previousData as any)?.results as
      | QueryResult
      | undefined;
    if (
      previousQueryResult &&
      previousQueryResult.data &&
      previousQueryResult.data.length > 0
    ) {
      // Calculate previous value using the same logic
      let previousValue = 0;
      const previousResultRow = previousQueryResult.data[0];

      if (previousResultRow && "value" in previousResultRow) {
        previousValue = Number(previousResultRow.value) || 0;
      } else {
        // Fallback logic for previous data
        const column = config.value_column;

        if (column === "*") {
          previousValue = previousQueryResult.data.length;
        } else {
          const previousValues = previousQueryResult.data
            .map((row: any) => row[column])
            .filter(
              (val: any) =>
                val !== null && val !== undefined && !isNaN(Number(val))
          ...
          switch (config.calculation.toLowerCase()) {
            case "sum":
              previousValue = previousValues.reduce(
                (sum: number, val: number) => sum + val,
                0
              );
              break;
            ...
            case "max":
              previousValue =
                previousValues.length > 0 ? Math.max(...previousValues) : 0;
              break;
            case "min":
              previousValue =
                previousValues.length > 0 ? Math.min(...previousValues) : 0;
              break;
            default:
              previousValue = previousValues.reduce(
                (sum: number, val: number) => sum + val,
  ...
  if (error) {
    return (
      <div className="bg-white border border-red-200 rounded-lg p-6">
        ...
        <div className="text-red-600 text-sm">Error loading KPI: {error}</div>
      </div>
    );
  }
